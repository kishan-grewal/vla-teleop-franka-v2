...
> XR teleoperation, data collection, and closed-loop Vision-Language-Action (VLA) policy deployment on a 7-DOF Franka Emika Panda, for autonomous electrical connector (MSD plug) insertion. Built in partnership with Extend Robotics.

![Python](https://img.shields.io/badge/python-3.12%20%7C%203.13-blue)
![C++](https://img.shields.io/badge/C%2B%2B-17-blue)
[![Built with LeRobot](https://img.shields.io/badge/built%20with-LeRobot-orange)](https://github.com/huggingface/lerobot)
...
| Path | Description |
|------|-------------|
| `cpp/teleop_bridge/` | Real-time C++ bridge: XR/policy command sources, planner, safety validation, libfranka control loop |
| `tools/run_smolvla_policy.py` | The deployment runner: loads a LeRobot policy and streams joint targets to the bridge over UDP |
| `tools/record_data_collection_session.py` | Launches synchronised robot + camera recorders for one collection session |
| `tools/record_robot_observations.py` | Records UDP robot observations and episode markers to JSONL |
| `tools/record_realsense_camera.py`, `tools/record_zed_camera.py` | Timestamped camera recorders for dataset sync |
| `tools/align_robot_camera_jsonl.py` | Nearest-neighbour timestamp alignment of robot state to camera frames |
| `tools/test_deploy_paths.py` | Hardware-free harness exercising both inference paths on CPU |
| `tools/plot_*.py`, `tools/live_teleop_debug.py` | Diagnostics: trajectory plots, live UDP observation monitor |
...
- **Output smoothing matters more than expected.** Light filtering (EMA) preserved the small corrective motions needed to seat the plug; heavier low-pass filtering removed them and insertion failed entirely.
...
This is a research deployment tied to specific hardware, not a turnkey package. Reproducing it requires:

- 7-DOF Franka Emika Panda arm + libfranka / real-time kernel
- Meta Quest 3 (teleoperation)
- Intel RealSense D405 and/or Stereolabs ZED-M cameras
- An NVIDIA GPU for inference (development used an RTX 4090)
- Python 3.12–3.13, a local LeRobot checkout, and the relevant camera SDKs